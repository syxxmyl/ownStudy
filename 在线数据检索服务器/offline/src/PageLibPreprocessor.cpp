...
void PageLibPreprocessor::readInfoFromFile()
{
	...
	if((!pageIfs.good()) || (!offsetIfs.good()))
	{
        ...
        logger->error("读取ripepage.lib和offset.lib失败");
	}
...
void PageLibPreprocessor::buildInvertIndexTable()
{
	...
	for(auto page : _pageLib)
	{
		...
		for(auto wordFreq : wordsMap)
		{
			_invertIndexTable[wordFreq.first].push_back(std::make_pair(
					page.getDocId(), wordFreq.second));//当前存放的还不是逆文档频率,而是该词的词频TF
		}
	}
	...
	int totalPageNum = _pageLib.size();//网页总数
	for(auto & item : _invertIndexTable)
	{	
		int df = item.second.size();//有多少篇文章包含了这个词
		double idf = log(static_cast<double>(totalPageNum)/ df + 0.05) / log(2);//求关键词item.first的逆文档频率
		
		for(auto & sitem : item.second)
		{
			double weight = sitem.second * idf;//在文档内出现的次数*逆文档频率得到权重值w
